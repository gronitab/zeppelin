...
const Graph = (() => {
  ...
  function init(container, clickHandler) {
  ...
  function update(snapshot) {
    ...
    const links = edges.filter(e => nodeMap[e.source] || nodeMap[e.source?.id])
      .map(e => ({
        ...e,
  ...
  function renderNodes(nodes) {
    ...
    // Update classes on existing nodes.
    nodeSel.attr('class', d => 'node node-' + d.type + ' state-' + d.state);

    // Apply breathing to working polecats.
    nodeSel.merge(nodeEnter)
      .classed('breathing', d => d.type === 'polecat' && d.state === 'working');

    // Apply pulse to hooked beads.
    nodeSel.merge(nodeEnter)
      .classed('pulse-yellow', d => d.type === 'bead' && d.state === 'hooked');
    ...
    nodeElements = nodeSel.merge(nodeEnter);
  }
